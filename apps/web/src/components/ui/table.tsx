...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        'border-b border-border/50 transition-colors hover:bg-surface/60 data-[state=selected]:bg-surface',
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      'h-11 px-4 text-start align-middle text-xs font-semibold text-muted uppercase tracking-wide [&:has([role=checkbox])]:pe-0',
      className,
    )}
    {...props}
  />

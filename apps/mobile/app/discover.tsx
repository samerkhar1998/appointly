...
// A single salon result card with logo, name, city, and a booking CTA.
// onPress navigates to the salon's booking flow.
...
// Public discovery screen — debounced search across all active public salons.
export default function DiscoverScreen() {
  const insets = useSafeAreaInsets(); // used for bottom padding only
  const [rawQuery, setRawQuery] = useState('');
  const [query, setQuery] = useState('');
  const [page, setPage] = useState(1);
  const debounceTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Debounces the search query by 400 ms.
  ...
  return (
    <View style={styles.root}>
      <ScreenHeader title={t('discover_title')} showBack />
